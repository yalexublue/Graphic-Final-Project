...
public:
    ...
    void update(const std::vector<T>& indices)
    {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(T) * indices.size(), indices.data(), GL_DYNAMIC_DRAW);
        cols = indices.size();
        rows = indices[0].length();
        check_gl_error();
    };
...
public:
  ...
  GLint bindVertexAttribArray(const std::string &name, VertexBufferObject& VBO) const;
    GLint bindElementAttribArray(const std::string &name, VertexBufferObject& EBO) const;
